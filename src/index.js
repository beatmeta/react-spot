...
export function installReactSpot(options = {}) {
...
export function findFiberFromElement(element) {
  let current = element;
...
export function getFiberFromDomNode(node) {
...
export function findSourceFromFiber(startFiber) {
  const seen = new Set();
  const queue = [];
...
export async function openSource(source, options = {}) {
...
export function createOpenInEditorUrl(source, options = {}) {
...
export function parseSourceFromStack(stack) {
...
export class ReactSpotError extends Error {
...
function parseSourceFromStackLine(line) {
  ...
  const location = cleaned.match(/\((?<location>.*:\d+:\d+)\)$/)?.groups?.location ??
    cleaned.match(/(?<location>(?:(?:webpack-internal:\/\/\/|webpack:\/\/\/|turbopack:\/\/\/|file:\/\/\/|https?:\/\/|[A-Za-z]:[\\/]|\/|\.\.?[\\/]).*|[^\s()]+\.[cm]?[jt]sx?|[^\s()]+\.mdx):\d+:\d+)\)?$/)?.groups?.location;
...
function isLikelyUserSource(fileName) {
  ...
  if (
    ...
    lower.includes("\\_next\\static\\chunks\\") ||
    lower.includes("/.next/static/chunks/") ||
    lower.includes("\\.next\\static\\chunks\\") ||
    lower.includes("/.next/dev/static/chunks/") ||
    lower.includes("\\.next\\dev\\static\\chunks\\") ||
